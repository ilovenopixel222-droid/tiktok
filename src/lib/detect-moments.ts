/**
 * Client-side viral moment detection and clip generation.
 * Runs entirely in the browser — no server call needed, avoids Vercel body size limits.
 */
...
const PATTERNS: Array<{ keywords: string[]; type: string; weight: number }> = [
  {
    keywords: [
      "crazy", "insane", "wild", "omg", "what the", "no way", "oh my god",
      "holy", "unreal", "are you serious", "you're kidding", "i can't believe",
      "shocking", "mind blowing", "jaw drop",
    ],
    type: "Shocking",
    weight: 1.2,
  },
  {
    keywords: [
      ...
      "bro what", "i'm dead", "that's so", "bruh", "lmao", "rofl",
    ],
    type: "Funny",
    weight: 1.15,
  },
  {
    keywords: [
      "disagree", "wrong", "debate", "argue", "but actually", "that's not",
      "i don't think", "you're wrong", "hold on", "wait what", "no no no",
      "here's the problem", "that's ridiculous",
    ],
    type: "Debate",
    weight: 1.1,
  },
  {
    keywords: [
      "story", "remember", "one time", "back when", "let me tell",
      "so basically", "what happened was", "true story", "i'll never forget",
      "picture this", "imagine this", "so check this out",
    ],
    type: "Storytelling",
    weight: 1.05,
  },
  {
    keywords: [
      "angry", "rage", "frustrated", "pissed", "mad", "furious",
      "so annoying", "sick of", "i hate", "this is bs", "fed up",
    ],
    type: "Rage",
    weight: 1.1,
  },
  {
    keywords: [
      "motivat", "inspire", "never give up", "you can", "believe in",
      "keep going", "don't quit", "push through", "you got this",
      "trust the process", "champion", "mindset",
    ],
    type: "Motivational",
    weight: 1.0,
  },
  {
    keywords: [
      "fail", "messed up", "disaster", "went wrong", "oops", "broken",
      "scuffed", "glitched", "crashed", "ruined", "mistake",
    ],
    type: "Stream Fail",
    weight: 1.15,
  },
  {
    keywords: [
      "emotional", "crying", "tears", "so sad", "heartbreak", "i miss",
      "it hurts", "i love you", "thank you so much", "that means",
      "grateful", "blessed",
    ],
    type: "Emotional",
    weight: 1.05,
  ...
  {
    keywords: [
      "controversial", "hot take", "unpopular opinion", "people don't realize",
      "nobody talks about", "here's the thing", "truth is", "wake up",
    ],
    type: "Controversial",
    weight: 1.1,
  },
  // Feature/capability discussion patterns (user-requested)
  {
    keywords: [
      "feature", "capability", "what it has", "what it doesn't", "what it needs",
      "missing", "limitation", "doesn't have", "needs to have", "should have",
      "compared to", "better than", "worse than", "unlike", "advantage",
      "update", "new in", "they added", "they removed", "changelog",
      "the one thing", "biggest problem", "best part", "worst part",
    ],
    type: "Feature Review",
    weight: 1.25,
  },
...
function computeViralScore(seg: ScoredSegment): number {
  ...
  const raw =
    (baseFactor + keywordFactor + sentimentFactor + confidenceFactor + lengthFactor) * weight;
...
function buildClipWindow(
  ...
  const text = words
    ...
    .map((w) => w.text)
    .join(" ");
  return { start: finalStart, end: finalEnd, text: text.substring(0, 500) };
}
...
function detectMomentsFromTranscript(
  ...
  for (const u of utterances) {
    ...
    for (const pattern of PATTERNS) {
      ...
      if (
        types.length > 0 &&
        !types.some((t) => t.toLowerCase().includes(pattern.type.toLowerCase()))
      )
        continue;
  ...
  // High-confidence sentiment segments
  for (const s of transcript.sentiment_analysis_results || []) {
    ...
    if (words.length > 0) {
      ...
      if (clipWindow) {
        ...
        if (
          types.length === 0 ||
          types.some((t) => t.toLowerCase().includes(momentType.toLowerCase()))
        ) {
          candidates.push({
...
function generateClipTitle(moment: TranscriptMoment): string {
  ...
  const prefixes: Record<string, string[]> = {
    ...
    "Feature Review": ["\u{1F50D} ", "Feature: ", "Review: "],
...
export function generateClipsFromTranscript(
  transcript: TranscriptData,
  momentTypes: string[],
  title: string,
  audioUrl: string,
  videoId?: string,
): { clips: GeneratedClip[]; clipsGenerated: number; videoId: string } {
  const vid = videoId || `vid_${Date.now()}`;
  const moments = detectMomentsFromTranscript(transcript, momentTypes);

  const clips: GeneratedClip[] = moments.map((moment, i) => {
  ...
  return { clips, clipsGenerated: clips.length, videoId: vid };
}
